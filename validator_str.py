...
# read_results - MODIFIED TO ACCEPT STRING INSTEAD OF FILE
def readresults(persons, hospitals, result_string):
    """
    Modified version that reads from a string instead of a file.
    ...
    result_string : str
        The solution string returned by my_solution()
    """
    ...
    # Split the string into lines instead of reading from file
    data = result_string.strip().split("\n")
...
def plot(_persons, _hospitals, result_string):
    """
    Modified to accept result_string instead of reading from file
    """
    # TODO: Plot bug
    ...
    # Use result_string instead of reading from file
    data_lines = result_string.strip().split("\n\n")
    if len(data_lines) > 1:
        data = data_lines[1].strip().split("\n")
    else:
        # Handle case where there's no double newline separator
        data = [
            line
            for line in result_string.strip().split("\n")
            if line.lower().startswith("ambulance")
        ]
    ...
    for i, (hosp, val) in enumerate(vals.items()):
        for pers in val:
            ax.plot(
                *zip(*process_list(hosp, pers)),
                label=hosp,
                color=colors[i % len(colors)],
            )
            ax.scatter(*zip(*pers), color=colors[i % len(colors)])
        ax.scatter(
...
def my_solution(pers, hosps):
    ...
    Your code SHOULD RETURN a string consisting of the solution as exactly the same as in the validator who outputs to result.txt
    IO Format:
    ...
    Return:
    -------
    A string containing the solution in the specified format.
    """
...
if __name__ == "__main__":
    ...
    # Get solution string from my_solution instead of reading from file
    solution_string = my_solution(persons, hospitals)

    # Pass the solution string to readresults instead of a filename
    readresults(persons, hospitals, solution_string)

    # Pass the solution string to plot as well
    # Comment the below line to disable plot
    plot(persons, hospitals, solution_string)
